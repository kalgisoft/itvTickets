...
namespace ItvTicketsService.Server.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpDelete("{id:int}")]
        public async Task<ActionResult<ApplicationUser>> UserDelete(int id)
        {
            try
            {
                ...
                if (!result.Succeeded)
                {
                    return NotFound($"User Id={id} has tickets");
                }
                else
                {
                    var user = await _userManager.FindByNameAsync(User.Identity.Name);
                    ...
                    Log lg = new Log();
                    lg.CreatedDate = DateTime.Now.ToString();
                    if (user != null) lg.UserId = user.Id;
                    lg.LogLevel = "Information";
                    lg.ExceptionMessage = $"Deleted user={userToDelete.UserName} (id={userToDelete.Id}) by {User.Identity.Name}";
                    lg.Source = "Server";
                    await _logManager.LogInsert(lg);
